...
# 20 日线斜率判定：拿当前 ma20 与 lookback 天前的 ma20 比。
# 涨幅 > GC_SLOPE_UP_MIN 记为“走上”，本策略只做“上”金叉。
# 注意：金叉当天 20 日线刚上拐，斜率很小，所以用较短 lookback + 接近 0 的阈值，
# 否则会把绝大多数刚形成的“上”金叉误判成“中/下”而漏掉。
GC_SLOPE_LOOKBACK = 3
GC_SLOPE_UP_MIN = 0.0  # 20 日线只要在上行（严格 > 0）就算“走上”

# 选股：现价（金叉当天收盘价）必须低于该值。
GC_MAX_PRICE = 500.0

# 短线止盈：买入后涨到 +8% 止盈（用户选区间中点）。
GC_TAKE_PROFIT = 0.08

# “有效跌破”均线的容差：收盘价低于均线 *(1 - margin) 才算跌破。
# 0.0 = 收盘只要低于均线就算跌破（可调大以过滤假跌破）。
GC_BREAK_MARGIN = 0.0

# 同时最多持有只数 + 每只目标仓位（等权）。
GC_MAX_HOLDINGS = 5
GC_POSITION_WEIGHT = 0.20
